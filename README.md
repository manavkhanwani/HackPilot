...
> Go from theme to pitch in minutes — with GROK 2.5 Flash (or a local Ollama model) doing the heavy lifting.
...
Select your language from the sidebar — GROK's multilingual capability handles translation natively. No external translation APIs required.
...
You can supply your own **GROK API key** directly in the sidebar instead of configuring secrets. Key resolution order:
1. Key entered in the sidebar (highest priority)
2. `GROK_API_KEY` in Streamlit secrets
...
Leave secrets empty and paste your key into the *GROK API Key* field in the app sidebar at runtime.
...
│   ├── ai_provider.py            # ★ Unified AI service layer (GROK + Ollama)
│   ├── language.py               # ★ Language enum & prompt instruction builders
│   ├── GROK.py                 # Legacy GROK wrapper (kept for compatibility)
...
- **Frontend:** Streamlit
- **LLM (cloud):** GROK 2.5 Flash (`google-genai`)
